...
import { MojProfilDialogComponent } from '../dialogs/moj-profil-dialog/moj-profil-dialog.component';
...
export class MojProfilComponent implements OnInit {
  ...
  public loadData() {
    console.log(this.Auth.vKorisnikIDVal);
    this.korisniciService.getKorisnikByID(this.Auth.vKorisnikIDVal).subscribe(data=>{
  ...
  public openDialog(flag: number, KorisnikID: number, Ime: string, Prezime: string, Jmbg: number, Telefon: string, Email: string, Lozinka: string, UlogaID: number) {
    const dialogRef = this.dialog.open(MojProfilDialogComponent,
